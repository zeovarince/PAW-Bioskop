...
// 4. Ambil Film (Now Showing & Coming Soon)
$query_now = "SELECT * FROM movies WHERE release_date <= '$today' ORDER BY release_date DESC LIMIT 4";
$tampil_now = mysqli_query($conn, $query_now);

$query_soon = "SELECT * FROM movies WHERE release_date > '$today' ORDER BY release_date ASC LIMIT 4";
$tampil_soon = mysqli_query($conn, $query_soon);
?>
    ...
    <div class="pt-20 pb-12">
            ...
            <div class="max-w-7xl mx-auto px-4 py-16 sm:px-6 lg:px-8 relative z-10">
                <div class="flex flex-col md:flex-row justify-between items-center">
                    <div class="mb-7 md:mb-10">
                        ...
                        <h2 class="text-4xl md:text-5xl font-bold mb-4">Halo Admin !</h2>
                        <p class="text-gray-400 max-w-xl text-lg">Cek laporan pembelian dan kelola jadwal tayang film</p>
                        <div class="mt-6 flex gap-3">
                            <a href="validation.php" class="bg-cinemaRed hover:bg-red-700 text-white px-6 py-3 rounded-lg font-bold shadow-lg transition transform hover:-translate-y-1">Cek Pesanan</a>
                            <a href="manage_movies.php" class="bg-gray-800 hover:bg-gray-700 text-white px-6 py-3 rounded-lg font-bold border border-gray-700 transition">Tambah Film</a>
                        </div>
                    </div>
                    <div class="grid grid-cols-1 gap-4 w-full md:w-auto">
                        <div class="bg-white/10 backdrop-blur-md border border-white/20 p-4 rounded-xl flex items-center min-w-[250px]">
                            <div class="p-3 bg-green-500/20 text-green-400 rounded-lg mr-4"><i class="ph ph-currency-dollar text-2xl"></i></div>
                            <div>
                        ...
                        <div class="bg-white/10 backdrop-blur-md border border-white/20 p-4 rounded-xl flex items-center min-w-[250px]">
                            <div class="p-3 bg-orange-500/20 text-orange-400 rounded-lg mr-4"><i class="ph ph-hourglass text-2xl"></i></div>
                            <div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-12">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                <?php if(mysqli_num_rows($tampil_now) > 0): ?>
                    <?php while($movie = mysqli_fetch_assoc($tampil_now)): ?>
            ...
            <!-- SECTION: COMING SOON -->
            <div class="mt-16 flex justify-between items-center mb-6 border-b border-gray-300 pb-4">
                <div>
                    <h3 class="text-2xl font-bold text-gray-900 flex items-center gap-2">
                        <i class="ph ph-hourglass-medium text-cinemaGold"></i> Segera Tayang
                    </h3>
                    <p class="text-gray-500 text-sm mt-1">Film yang dijadwalkan akan tayang di masa depan.</p>
                </div>
            </div>

            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                <?php if(mysqli_num_rows($tampil_soon) > 0): ?>
                    <?php while($movie = mysqli_fetch_assoc($tampil_soon)): ?>
                        <div class="bg-white rounded-xl shadow-sm hover:shadow-xl transition duration-300 overflow-hidden border border-gray-100 group opacity-90 hover:opacity-100">
                            <div class="relative aspect-[2/3] bg-gray-200 overflow-hidden">
                                <img src="../assets/images/<?= $movie['poster'] ?>" 
                                     alt="<?= $movie['judul'] ?>" 
                                     class="w-full h-full object-cover grayscale group-hover:grayscale-0 transition duration-500"
                                     onerror="this.src='https://via.placeholder.com/300x450?text=No+Poster'">
                                <div class="absolute top-2 left-2 bg-cinemaRed text-white text-[10px] font-bold px-2 py-1 rounded shadow uppercase">
                                    Coming Soon
                                </div>
                            </div>
                            <div class="p-4">
                                ...
                                <div class="flex justify-between items-center mt-2 text-sm text-gray-500">
                                    <span><i class="ph ph-calendar text-cinemaRed"></i> <?= date('d M Y', strtotime($movie['release_date'])) ?></span>
                                </div>
                                <a href="manage_movie.php?id=<?= $movie['Id_movie'] ?>" class="mt-4 block w-full text-center bg-gray-100 hover:bg-gray-800 hover:text-white text-gray-600 py-2 rounded-lg text-sm font-semibold transition">
                                    Edit Info
                                </a>
                            </div>
                        </div>
                ...
                <?php else: ?>
                    <div class="col-span-4 text-center py-10 bg-white rounded-xl border border-dashed border-gray-300">
                        <p class="text-gray-500">Belum ada jadwal film mendatang.</p>
                    </div>
                <?php endif; ?>
            </div>
